...
class LocalContractEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.record = {"version": "v0.4.3", "generateMainContract": {
    ...
    def test_valid_evidence_verifies_real_input_files_offline(self):
        validate(self.record, "v0.4.3", root=self.root, contract_only=True)
    ...
    def test_future_versions_cannot_use_local_mode(self):
        record = copy.deepcopy(self.record)
        record["version"] = "v0.4.4"
        with self.assertRaises(ValueError):
            validate(record, "v0.4.4", root=self.root, contract_only=True)
